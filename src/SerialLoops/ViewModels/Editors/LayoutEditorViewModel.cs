...
public class LayoutEditorViewModel : EditorViewModel
{
    ...
    public LayoutEditorViewModel(ReactiveItemDescription item, MainWindowViewModel window, ILogger log) : base(item, window, log)
    {
        ...
        _mainWindow = window;
        LayoutEntries = new(_layout.Layout.LayoutEntries.Skip(_layout.StartEntry).Take(_layout.NumEntries).Select((_, i) => new LayoutEntryWithImage(Description, _layout, i + _layout.StartEntry, history: _history)));
